...
\begin{example}
    Let $V = \mathbb{F}[x]$ and for $i \in \mathbb{N} \cup \{0\}$, let $W_i := \mathrm{Span}\{x^i\}$. Then $\sum_{i \in \mathbb{N} \cup \{0\}} W_i = \bigoplus_{\mathbb{N} \cup \{0\}} W_i$ is a direct sum -- indeed, suppose
    $$w_{i_1} = \alpha_1 x^{i_1} \in W_{i_1}, \quad w_{i_2} = \alpha_1 x^{i_2} \in W_{i_2}, \quad \dots, \quad w_{i_k} = \alpha_k x^{i_k} \in W_{i_k}$$
    ($\alpha_i \in \mathbb{F}$) be such that
    $$w_{i_1} + w_{i_2} + \dots + w_{i_k} = \alpha_1 x^{i_1} + \alpha_2 x^{i_2} + \dots + \alpha_k x^{i_k} = {\bf 0}.$$
    where ${\bf 0} \in V$ is the zero polynomial. Without loss of generality, assume $i_1 < i_2 < \dots < i_k$. Then the above equation implies that
    $$\alpha_1 x^{i_1} + \alpha_2 x^{i_2} + \dots + \alpha_k x^{i_k} = 0 + 0x + 0x^2 
    +\dots + 0x^{i_1} + \dots + 0x^{i_2} + \dots + 0x^{i_k}$$
    By comparing the coefficients of $x^{i_l}$, one can conclude that $\alpha_l = 0$ for all $1 \leq l \leq k$, and hence $w_{i_l} = {\bf 0}$ for all $l$.
\end{example}
